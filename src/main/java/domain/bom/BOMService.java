...
public class BOMService {
    ...
    public BOM calculateBom(HashMap<Category, ConstructionPart> constructionList, TreeSet<Integer> widths, TreeSet<Integer> lengths/*, UsersChoice usersChoice, ConstructionFactory constructionFactory*/) {
        ...
        if (roof.isFlat()) {
            ...
            for (int i = 1; i <= trapezPlates.size(); i++) {
                for (Map.Entry meassuresMaterialAndQnty : trapezPlates.get(i).entrySet()) {
                    ...
                    if (quantity != 0)
                        bom.addItem(new BOMItem(roof.getCladding(), quantity, length, "tagplader monteres på spær", width));
                }
            }
            ...
            bom.addItem(new BOMItem(roof.getCladding(), pitchedRoofMaterialCalculator.amountOfRoofTiles(), tilesMateriallength, "monteres på taglægter", tilesMaterialWidth));
            bom.addItem(new BOMItem(roof.getCladding(), pitchedRoofMaterialCalculator.quantityRygsten(), tilesMateriallength, "monteres på toplægte med medfølgende beslag se tagstens vejledning", tilesMaterialWidth));

            bom.addItem(new BOMItem((Material) materialMapRoofWood.get("Færdigskåret (byg-selv-spær) 8 stk. 30"), pitchedRoofMaterialCalculator.spærQuantity(), pitchedRoofMaterialCalculator.spærFullQuantityOfPlanksTotal(), "byg-selv spær (skal samles)",width));
            bom.addItem(new BOMItem((Material) materialMapRoofWood.get("trykimp. Bræt 19"), pitchedRoofMaterialCalculator.gavlOverlayQuantity(roofGableMatertialWidth,pitchedRoof.getWidth()), pitchedRoof.getWidth(),"beklædning af gavle 1 på 2",roofGableMatertialWidth));
            bom.addItem(new BOMItem((Material) materialMapRoofWood.get("taglægte T1 38"), pitchedRoofMaterialCalculator.qntyOfRoofFirstLathBothSides(), pitchedRoof.getWidth(),"toplægte til montering af rygsten lægges i toplægte holder", roofLathMaterialWidth));
            bom.addItem(new BOMItem((Material) materialMapRoofWood.get("taglægte T1 38"), pitchedRoofMaterialCalculator.qntyOfRoofLathBothSides(), pitchedRoof.getWidth(),"til montering på spær, "+ ((pitchedRoofMaterialCalculator.qntyOfRoofLathBothSides())/2) + " rækker lægter på hver skiftevis 1 hel & 1 halv lægte", roofLathMaterialWidth));
            bom.addItem(new BOMItem((Material) materialMapRoofWood.get("trykimp. Bræt 25"), pitchedRoofMaterialCalculator.amountOfVindskeder(), roofWidthSurface,"Vindskeder på rejsning",roofVindskedeMaterialWidth));
            bom.addItem(new BOMItem((Material) materialMapRoofWood.get("trykimp. Bræt 25"), pitchedRoofMaterialCalculator.amountOfStern(), roofLengthSurface,"Sternbrædder til siderne Carport del", roofSternMatertialWidth));
        }
        ...
        bom.addItem(new BOMItem((Material) caportMaterialMap.get( "trykimp. Stolpe 97"), carportMaterialCalculator.qntyPost(carport.getWidth(),carport.getLength()), carport.getHeight(),"Stolper nedgraves 90 cm. i jord + skråstiver", postWidth));
        bom.addItem(new BOMItem((Material) caportMaterialMap.get("spærtræ ubh. 45"), carportMaterialCalculator.remQnty(), carportMaterialCalculator.remLength(carport), "Remme i side, sadles ned i stolper Carport del", widthMaterialRem));
        ...
        if (shed != null) {
            ...
            bom.addItem(new BOMItem((Material) shedMaterialMap.get("trykimp. Bræt 25"), shedMaterialCalculator.getStern().quantity(), shedMaterialCalculator.getStern().length(), shedMaterialCalculator.getStern().description("Sternbrædder til siderne - Skur del (deles)"), width));
            ...
            bom.addItem(new BOMItem(claddingMaterial, claddingShedCalculatedLengths.quantity()*2, claddingShedCalculatedLengths.length(), claddingShedCalculatedLengths.description("Beklædning skur sider"), width));
            bom.addItem(new BOMItem(claddingMaterial, claddingShedCalculatedWidths.quantity()*2, claddingShedCalculatedLengths.length(), claddingShedCalculatedLengths.description("Beklædning skur foran og bagved - dør skæres til efter ønske om placering"), width));
        }
        ...
        if (carport.getCladding() != null) {
            bom.addItem(new BOMItem(claddingMaterial, claddingMaterialCalculatorFront.quantity(), claddingMaterialCalculatorFront.length(), claddingMaterialCalculatorFront.description("Beklædning af carport bag til"), widthMaterialRem));
            bom.addItem(new BOMItem(claddingMaterial, claddingMaterialCalculatorSide.quantity()*2, claddingMaterialCalculatorSide.length(), claddingMaterialCalculatorSide.description("Beklædning af carportvægge på siderne"), widthMaterialRem));
        }
//TODO
        return bom;
